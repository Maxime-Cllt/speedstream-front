<script lang="ts">
	...
	let valueClasses = $derived(isNewRecord ? 'text-purple-500' : 'text-foreground');
...
<Card class={cardClasses}>
	...
	{#if isNewRecord}
		<div
			class="absolute top-0 left-0 h-1 w-full bg-gradient-to-r from-purple-600 to-purple-400"
		></div>
	{:else if isPersonalBest}
		<div
			class="absolute top-0 left-0 h-1 w-full bg-gradient-to-r from-emerald-500 to-emerald-400"
		></div>
	{/if}

	<CardHeader class="relative z-10 flex flex-row items-center justify-between pt-5 pb-2">
		<h3 class={titleClasses}>
			{title}
		</h3>

		<div
			class="flex h-8 w-8 items-center justify-center rounded-md transition-colors {iconBgClasses}"
		>
			{#if isNewRecord}
				<Timer class="h-4 w-4" />
			{:else}
		...
		<div class="flex flex-col gap-1">
			<div class="flex items-baseline gap-2">
				...
				{#if trend && trendValue && !isNewRecord}
					<span
						class="flex items-center gap-0.5 rounded px-1.5 py-0.5 font-mono text-sm font-medium {trendColor}"
					>
			...
			<div class="mt-2 min-h-[20px]">
				{#if isNewRecord}
					<span
						class="inline-flex animate-pulse items-center rounded bg-purple-500 px-2 py-0.5 font-mono text-[10px] font-bold tracking-widest text-white uppercase"
					>
